...
const NavigationComponent = props => {
    ...
    //handleSignOut created as a functional component so to call i =t we dont need the word "this" (It's not a class) 
    const handleSignOut = () => {
        axios.delete("https://api.devcamp.space/logout", {withCredentials: true}).then(response => {
           if (response.status === 200) {//so the promise is working
           props.history.push("/");// Take the user to homepage
